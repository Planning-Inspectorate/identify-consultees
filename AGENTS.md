...
- [ ] Dependency changes obey the npm dependency policy below (exact pins, lockfile integrity, cooldown).
...
## npm dependency policy (security)

This repository pins npm dependencies tightly to reduce supply-chain risk. Agents and contributors must follow this policy when adding or upgrading packages.

### Why we pin exactly (no `^`, `~`, or `*`)

npm’s default semver ranges (for example `^1.2.3`) allow newer compatible releases to be selected whenever the lockfile is regenerated. That is convenient, but it widens the window in which a compromised or buggy release can enter the tree without an intentional upgrade review.

Exact pins (`1.2.3`) mean:

- Installs are reproducible from `package.json` + `package-lock.json`.
- Upgrades are deliberate PR changes, not accidental float.
- Reviewers can see the precise version being adopted.

Historical npm incidents (account takeovers, typosquatting, malicious maintainer releases, and compromised publish credentials) have shown that “latest compatible” resolution can pull malware into CI and developer machines within hours of publish. Pinning does not stop a bad version from being chosen deliberately, but it stops silent drift and makes every bump reviewable.

### Why we require SHA-512 integrity in the lockfile

`package-lock.json` records an `integrity` field (SHA-512) for each resolved tarball. `npm ci` verifies that hash against the downloaded artifact. That protects against:

- Tampered or substituted tarballs for a given version.
- Mirror / CDN inconsistencies.
- Some classes of man-in-the-middle substitution.

Always commit an up-to-date `package-lock.json` and install in CI with `npm ci` (already used in Azure Pipelines). Do not delete integrity fields or hand-edit resolved URLs.

### Why we enforce a 7-day cooldown

Brand-new package versions are often where malware and broken publishes are discovered first. A **7-day cooldown** means we do not adopt a version until it has been published on the npm registry for at least seven days.

This is enforced in two places:

1. **Dependabot** — `.github/dependabot.yml` sets `cooldown.default-days: 7` so automated update PRs wait before proposing fresh releases.
2. **CI policy check** — `npm run check-npm-dependency-policy` queries the registry publish time for every locked package version and fails if any are newer than seven days.

Cooldown and automation reduce the chance of being first-line victims of a just-published malicious release while still allowing timely security upgrades after the cooldown (and immediate action when a known vulnerability requires a specific version — document exceptions in the PR if a break-glass upgrade is required).

### Automation in this repo

| Mechanism                             | Purpose                                                                                 |
| ------------------------------------- | --------------------------------------------------------------------------------------- |
| `.npmrc` → `save-exact=true`          | New `npm install <pkg>` writes exact versions into `package.json`.                      |
| `npm run pin-npm-dependencies`        | Rewrites workspace `package.json` files to exact versions from `package-lock.json`.     |
| `npm run check-npm-dependency-policy` | Fails on version ranges, missing/non-sha512 integrity, or versions younger than 7 days. |
| PR pipeline step                      | Runs the policy check after `npm ci`.                                                   |
| Dependabot cooldown                   | Avoids opening update PRs for packages published within 7 days.                         |

### Agent rules for dependencies

- Never add or restore range operators (`^`, `~`, `*`, `>`, `>=`, etc.) in any workspace `package.json`.
- After changing dependencies: run `npm install`, ensure the lockfile is committed, then run `npm run check-npm-dependency-policy`.
- Prefer `npm run pin-npm-dependencies` if a manifest accidentally regained ranges.
- Do not bypass the cooldown by manually forcing a <7-day-old release without an explicit user request and a clear PR justification (for example an actively exploited CVE with no older fixed release).
- Keep using `npm ci` in CI; do not switch CI installs to `npm install` (which can mutate the lockfile).
