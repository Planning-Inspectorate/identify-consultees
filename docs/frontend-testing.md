...
2. **Browser e2e + a11y** — Playwright projects `chromium-e2e` and `chromium-a11y` (auth disabled test server)
...
Runs manage tests with `--test-coverage-lines=100` and `--test-coverage-functions=100` for `apps/manage/src/**`, excluding:

- `server.ts` (process entry)
- `util/build.ts` (asset build script; covered by `npm run build` in CI)
- `*.test.*`

`npm run test-coverage` runs monorepo coverage, Playwright e2e/a11y, then this frontend gate.
...
| Script                           | Purpose                                 |
| -------------------------------- | --------------------------------------- |
| `npm test`                       | Unit/integration + Playwright e2e/a11y  |
| `npm run test:e2e`               | Playwright e2e + browser a11y only      |
| `npm run test:frontend-coverage` | Manage 100% line/function coverage gate |
