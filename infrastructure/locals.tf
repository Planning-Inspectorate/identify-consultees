locals {
  ...
  short_resource_suffix = var.environment == "training" ? "consultees-${local.environment}" : local.resource_suffix
  ...
  tech_emails = [for rec in azurerm_monitor_action_group.consultees_tech.email_receiver : rec.email_address]
  action_group_ids = {
    tech            = azurerm_monitor_action_group.consultees_tech.id
    service_manager = azurerm_monitor_action_group.consultees_service_manager.id
